  ...
  def self.configure(config, settings)
    ...
    if settings.has_key?('sources')
      config.vm.provision 'shell' do |s|
        ...
        s.path = script_dir + '/sources-list/' + settings['sources'] + '.sh'
      end
    end
    ...
    if settings.include? 'sites'
      ...
      settings['sites'].each do |site|
        ...
        config.vm.provision 'shell' do |s|
          ...
          # specific site type script (defaults to apache)
...
#       s.inline = 'sudo chown -R vagrant:vagrant /usr/local/bin && sudo -u vagrant /usr/local/bin/composer self-update --no-progress && sudo chown -R vagrant:vagrant /home/vagrant/.composer/'
      s.inline = 'sudo chown -R vagrant:vagrant /usr/local/bin && sudo -u vagrant /usr/local/bin/composer self-update --2 && sudo chown -R vagrant:vagrant /home/vagrant/.composer/'
      s.privileged = false
    end
